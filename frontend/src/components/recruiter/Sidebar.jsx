...
import { Brain, FileText, Search, Calendar, Clock, BarChart3, LogOut, ChevronLeft, ChevronRight } from 'lucide-react';
import { Link } from 'react-router-dom';
import { createPageUrl } from '@/utils';

const menuItems = [
  { icon: FileText,  label: 'Resume Analysis', id: 'analysis'  },
  { icon: Search,    label: 'Talent Search',   id: 'talent'    },
  { icon: Calendar,  label: 'Scheduled',       id: 'scheduled' },
  { icon: Clock,     label: 'Waitlist',        id: 'waitlist'  },
  { icon: BarChart3, label: 'AI Analytics',    id: 'analytics' },
];
...
export default function Sidebar({ activeTab, setActiveTab, collapsed, setCollapsed }) {
  return (
    <motion.aside
      ...
      className="h-screen sticky top-0 flex flex-col border-r z-40 nh-scrollbar"
      style={{
    ...
    >
        ...
        {menuItems.map((item) => {
          ...
          return (
            <button
              ...
              style={{
                background: isActive ? 'var(--nh-primary-light)' : 'transparent',
              }}
            >
              {isActive && (
      ...
      {/* ── Bottom: Exit only (ThemeToggle moved to header) ── */}
